...
int main(int argc, char * argv[])
{
  ...
  */ 
  ...
  for (std::vector<double>::size_type i=0; i != angle_len; i++){
    ...
    ltop_p_rear.at(i) = ltop_p_rear.at(i) * power_phi;
    lmid_p_rear.at(i) = lmid_p_rear.at(i) * power_phi;
    lbtm_p_rear.at(i) = lbtm_p_rear.at(i) * power_phi;
    rtop_p_rear.at(i) = rtop_p_rear.at(i) * power_phi;
    rmid_p_rear.at(i) = rmid_p_rear.at(i) * power_phi;
    rbtm_p_rear.at(i) = rbtm_p_rear.at(i) * power_phi;
  ...
  for (size_t i=0; i != angle_len; i++){
    ...
    ltop_p_rear.at(i) = ltop_p_rear.at(i)/60;
    lmid_p_rear.at(i) = lmid_p_rear.at(i)/60;
    lbtm_p_rear.at(i) = lbtm_p_rear.at(i)/60;
    rtop_p_rear.at(i) = rtop_p_rear.at(i)/60;
    rmid_p_rear.at(i) = rmid_p_rear.at(i)/60;
    rbtm_p_rear.at(i) = rbtm_p_rear.at(i)/60;
  ...
  for (size_t i=0; i != angle_len; i++){
    ...
    for (size_t j=0; j != joint_names.size()/3; j++){
        if (j<=2){
          // Right quadruped torotto
          point.positions[3*j] = r_r1.at(i);
          point.positions[3*j+2] = -1*r2;

          if (j == 0){
            point.positions[3*j+1] = -1*rtop_p_rear.at(i);
          }
            ...
            point.positions[3*j+1] = lbtm_p_rear.at(i);
          }
        }
        else{
          // Left back quadruped torotto
